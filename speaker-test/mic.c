...
uint32_t goertzel_detect(const double *tones,   int    n_tones,
                         double        noise_hz, double thresh_mul, int debug, double *mag) {
...
void notmain(void) {
    ...
    while (1) {
        ...
        goertzel_detect(tones, n_tones, 500.0, 4.0, 0, &mag[0][0]);
        
        // this should be 0xFF
        uint32_t mask1  = goertzel_detect(tones, n_tones, 500.0, 4.0, 0, &mag[0][0]);
        ...
        for(int i=0;i<num_bytes;i++) {
            buf[i] = goertzel_detect(tones, n_tones, 500.0, 4.0, 0, &mag[i][0]);
        }
        break;
    }
    int count = 0;
